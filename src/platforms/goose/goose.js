...
module.exports = class goose {
  ...
  async getAddressFarms(address) {
    const cacheItem = this.cache.get(`getAddressFarms-goose-${address}`);
    if (cacheItem) {
      return cacheItem;
    }
    ...
    const vaultCalls = (await this.getFarms())
      ...
      .map(farm => {
        const vault = new Web3EthContract(goose.MASTER_ABI, goose.MASTER_ADDRESS);
        return {
          poolUserInfoMap: vault.methods.userInfo(farm.raw.pid, address),
          pendingEgg: vault.methods.pendingEgg(farm.raw.pid, address),
          id: farm.id.toString()
        };
      });
    ...
    const result = farmCalls
      .filter(
        v =>
          ...
          new BigNumber(v.pendingEgg || 0).isGreaterThan(0)
      )
      .map(v => v.id);

    this.cache.put(`getAddressFarms-goose-${address}`, result, { ttl: 300 * 1000 });

    return result;
  }

  async getFarms(refresh = false) {
    const cacheKey = "getFarms-goose";
    ...
    const result = rawPools.map(farm => {
      const item = {
        ...
        link: "https://www.goosedefi.com/farms",
        has_details: true,
        extra: {}
      };
      ...
      item.extra.transactionAddress = goose.MASTER_ADDRESS;
  ...
  async getYieldsInner(address, addresses) {
    ...
    const resultFarms = calls
      ...
      .map(call => {
        ...
        const rewards = call.pendingEgg || 0;
        ...
        if (rewards > 0) {
          ...
          const priceReward = this.priceOracle.findPrice("egg");
          if (priceReward) {
